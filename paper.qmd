...
records = gillespymax.history.read_records("vignette/sample_results.h5")
df = next(iter(records.values()))

# need to pivot longer to get into a plottable format
df_long = df.select(list('tSEIRDTQ')).unpivot(
  index='t',
  variable_name='state',
  value_name='count'
)

chart = df_long.plot.line(x='t', y='count', color='state')
