...
def create_twolayer_bipartite_network(
    n_indvs, n_hh, n_comm, p_comm, seed=None
) -> nx.Graph:
    ...
    # add demography to this layer
    for _, node_attrs in household.nodes.items():
        if node_attrs["bipartite"] == 0:
            node_attrs["demography"] = rng.integers(0, 4)

    # group 2 layer is the community layer
    community = nx.bipartite.generators.preferential_attachment_graph(
        aseq=[n_comm] * n_indvs, p=p_comm, create_using=nx.Graph, seed=rng
    )

    # relabel nodes
    household = nx.relabel_nodes(
        household, {i: (i if i < n_indvs else f"HH{i}") for i in household.nodes}
    )
    community = nx.relabel_nodes(
        community, {i: (i if i < n_indvs else f"CC{i}") for i in community.nodes}
    )
...
def paper_network() -> nx.Graph:
    return create_twolayer_bipartite_network(
        n_indvs=100, n_hh=25, n_comm=2, p_comm=0.25, seed=0x12B342B71
    )
